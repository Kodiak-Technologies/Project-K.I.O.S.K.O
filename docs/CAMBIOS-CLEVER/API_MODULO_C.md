...
> rama, por eso no se usa `alembic upgrade` ahí; las migraciones de Módulo C existen igual para
> entornos limpios).
...
| GET | `/caja/turnos/{id}/movimientos` | autenticado | RASTRO del turno: ventas y reversos (modal del ADMIN) |
...
`GET /caja/resumen` →
```json
{ "turno": {...}, "efectivo_esperado": 115.5,
  "desglose": { "monto_inicial": 100.0, "ventas_efectivo": 55.5,
                "devoluciones_efectivo": 40.0 },
  "totales_por_metodo": { "EFECTIVO": 55.5, "YAPE": 38.5 },
  "total_vendido": 94.0, "numero_ventas": 6 }
```
...
metodo_pago, pagos:[{metodo, monto, es_efectivo, monto_recibido, vuelto}], vuelto, vendedor,
...
| PATCH | `/metodos-pago/{id}` | `metodos_pago.gestionar` | `{nombre?, activo?}` (EFECTIVO no se desactiva) |

`es_efectivo = true` marca dinero FÍSICO: es lo único que cuenta para el arqueo (RF-17).

## 4. Permisos que el Módulo C agregó al seed

`ventas.devolver`, `metodos_pago.gestionar` — y `ventas.anular` pasó a estar disponible para
CAJERO (HU-C08: el reverso lo hace el cajero y el CONTROL es el rastro; la dueña puede quitárselo
sin redeploy desde `PUT /roles/{id}/permisos`). Avisado a Matías según su guía.

## 5. Notas de integración (para Brayan y Fabrizio)
