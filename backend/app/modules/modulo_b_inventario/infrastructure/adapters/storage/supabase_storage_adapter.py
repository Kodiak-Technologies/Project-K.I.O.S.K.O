...
#   - SUPABASE_SIGNED_URL_TTL=31536000  # opcional, segundos (default: 1 año)
#
# Decisiones:
#   - D-T01 (override PR2): se usa httpx en vez del SDK (no se instaló supabase-py).
#   - Q2 (proposal): para bucket `boletas` (privado) → signed URL; para
#     `productos` (público) → URL pública. El TTL de la firma es configurable y
#     por defecto de 1 año: la URL queda guardada en `foto_boleta_url` y con las
#     24 h originales la boleta dejaba de verse al día siguiente (HU-B07/B08).
#     Además `refirmar()` permite regenerar la URL a partir del path.
...
class SupabaseStorageAdapter(StoragePort):
    ...
    # HU-B07/B08: la URL de la boleta se guarda de forma permanente en
    # `solicitudes_ingreso.foto_boleta_url`. Con las 24 h originales, la
    # administradora no podía ver la boleta de una solicitud del día anterior.
    TTL_FIRMA_POR_DEFECTO = 31_536_000  # 1 año
    ...
    def __init__(self) -> None:
        ...
        try:
            self._ttl_firma = int(
                os.getenv("SUPABASE_SIGNED_URL_TTL", str(self.TTL_FIRMA_POR_DEFECTO))
            )
        except ValueError:
            self._ttl_firma = self.TTL_FIRMA_POR_DEFECTO
    ...
    async def subir(
        ...
        if carpeta == "boletas":
            url_final, expires_at = await self._firmar(bucket, path)
        else:
    ...
    async def refirmar(
        ...
        Permite recuperar el acceso a una boleta cuya URL firmada venció
        (HU-B07: 'revisar la solicitud junto con la boleta', sin límite de días).
        """
        if carpeta not in self._CARPETAS_VALIDAS:
            raise ValidacionError(
                f"Carpeta inválida. Permitidas: {', '.join(self._CARPETAS_VALIDAS)}."
        ...
        path = path.lstrip("/")
        if ".." in path:
            raise ValidacionError("Path inválido.")
        if carpeta == "boletas":
            url_final, expires_at = await self._firmar(bucket, path)
        else:
            url_final = f"{self._url}/storage/v1/object/public/{bucket}/{path}"
    ...
    async def _firmar(self, bucket: str, path: str) -> tuple[str, datetime]:
        """POST /object/sign/{bucket}/{path} → URL firmada + vencimiento."""
        signed_url = f"{self._url}/storage/v1/object/sign/{bucket}/{path}"
        try:
            async with httpx.AsyncClient(timeout=15.0) as cliente:
                respuesta_signed = await cliente.post(
                    ...
                    json={"expiresIn": self._ttl_firma},
                )
        except httpx.HTTPError as exc:
            raise ErrorDeDominio(
                f"No se pudo generar la URL firmada: {exc}"
            ) from exc
        if respuesta_signed.status_code == 404:
            raise NoEncontradoError("El archivo no existe en Storage.")
        ...
        return url_final, datetime.now(timezone.utc) + timedelta(seconds=self._ttl_firma)
